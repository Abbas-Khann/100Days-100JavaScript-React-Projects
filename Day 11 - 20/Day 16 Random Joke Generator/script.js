...
// When we deal with API data we want our function to be work seamless thus we would make function async and it will return a promise.
async function getJoke() {
    ...
    // an extra feature we have added :) let's get the joke id as well for that particular joke 
    jokeId.innerHTML = `Joke ID: ${data.value.id}`;
    
    // we also want to get the category for that joke but the joke we get sometime doesn't have any catogory.
    // So we would provide a condition, If the category property has no value which means the length is 0 we wouild just display "lame" as category.
